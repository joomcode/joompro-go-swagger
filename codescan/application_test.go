...
func loadPetstorePkgsCtx(t testing.TB) *scanCtx {
	...
	sctx, err := newScanCtx(&Options{
		Packages: []string{"github.com/ssfilatov/go-swagger/fixtures/goparsing/petstore/..."},
	})
...
func TestAppScanner_Definitions(t *testing.T) {
	doc, err := Run(&Options{
		Packages:   []string{"github.com/ssfilatov/go-swagger/fixtures/goparsing/bookings/..."},
		ScanModels: true,
	})
	require.NoError(t, err)
	if assert.NotNil(t, doc) {
